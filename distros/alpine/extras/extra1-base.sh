...
commit_patch(){
  ...
  local PACKAGES=(curl wget sudo nano vim util-linux ca-certificates openssl bash openssh)
  ...
  #Base Packages
  utils.lxc.attach apk upgrade -U --available
  utils.lxc.attach apk add ${PACKAGES[*]}
  
  #OPENSSH
  utils.lxc.attach rc-update add sshd
  utils.lxc.attach rc-status
